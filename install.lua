...
local function installFile( file )
	local download = http.get("https://raw.github.com/Logotocom-OS/GDProKid-OS/"..file) --This will make 'download' hold the contents of the file.
        local handle = download.readAll() --Reads everything in download
download.close() --remember to close download!

local file = fs.open(file,"w") --opens the file 'startup' with the permissions to write.
file.write(handle) --writes all the stuff in handle to the file 'startup'.
file.close() --remember to close download
end
-- Main
function display( _sText )
  term.clear()
...
local mainTitle = 'GDProKid OS Installer'
...
function Draw()
        ...
        term.write(subTitle)
        term.setCuraorPos( w/2 - 7, h -1)
        term.setTextColoir(colours.lightBlue)
        term.write( "Copyright 2020"
end
 
tArgs = {...}
Settings = {
        InstallPath = '/gdprokidOS', --Where the program's installed, don't always asume root (if it's run under something like OneOS)
        Hidden = true, --Whether or not the update is hidden (doesn't write to the screen), useful for background updates
        GitHubUsername = 'Logotocom-OS', --Your GitHub username as it appears in the URL
        GitHubRepoName = 'GDProKid-OS', --The repo name as it appears in the URL
